    ...
    <div ref={rootRef} className="relative space-y-1">
      ...
      {open && !disabled && (
        <ul
          id={listId}
          role="listbox"
          className="absolute z-30 mt-1 max-h-72 sm:max-h-80 w-full overflow-y-auto overscroll-contain rounded-lg border border-border bg-card shadow-lg py-1 scrollbar-thin"
        >
          {filtered.length === 0 ? (
            <li className="px-3 py-3 text-xs text-muted-foreground text-center">
              Sin materiales con saldo disponible en este proyecto
            </li>
          ) : (
            <>
              ...
              <li className="px-3 py-1.5 text-[11px] text-muted-foreground text-center bg-muted/40 border-t border-border/60 sticky bottom-0 backdrop-blur-xs select-none">
                {filtered.length === 1
                  ? '1 material con saldo disponible'
                  : `${filtered.length} materiales con saldo disponible`}
              </li>
            </>
          )}
        </ul>
