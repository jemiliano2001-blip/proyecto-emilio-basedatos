...
export function SolicitudesWorkbenchNav({
  ...
  return (
    <aside className="hidden lg:flex lg:flex-col min-h-0 w-full max-w-[340px] shrink-0">
      <div className="sticky top-[calc(3.5rem+env(safe-area-inset-top,0px))] max-h-[calc(100dvh-5rem)] flex flex-col rounded-xl border border-gray-200 bg-white overflow-hidden">
        <div className="px-3 py-2.5 border-b border-gray-200 bg-paper flex items-center justify-between gap-2">
          <p className="text-xs font-bold uppercase tracking-wide text-ink">Cola</p>
          <Link
            href="/solicitudes"
        ...
        >
          {items.length === 0 ? (
            <p className="p-4 text-sm text-gray-500">Sin otras requisiciones.</p>
          ) : (
            items.map((s) => {
              ...
              return (
                <Link
                  ...
                  className={
                    active
                      ? 'block px-3 py-3 bg-slate-50 border-l-2 border-l-accent'
                      : 'block px-3 py-3 hover:bg-slate-50 border-l-2 border-l-transparent'
                  }
                >
